...
export default function TacDashboard({ onBack, initialTab = 'summary', initialFilterNsp = false }) {
    ...
    async function loadData() {
      try {
        ...
        const cleaned = data.records.map((r, idx) => {
          ...
          // Loop through all 4 description columns to count all exams in this single visit
          for (let i = 1; i <= 4; i++) {
  ...
  const stats = useMemo(() => {
    ...
    // TAC lost capacity (typically 20 min per visit)
  ...
  // YoY calculation helper for TAC
  const getYoYStats = (key) => {
    let currentCount = 0;
    filteredData.forEach(r => {
      if (key === 'total') currentCount++;
      if (key === 'totalExams') currentCount += r.examsCount;
      if (key === 'urgentCount' && r.procedencia_de_derivaci_n === 'Urgencia') currentCount++;
      if (key === 'closedCount' && r.procedencia_de_derivaci_n === 'Atención Cerrada') currentCount++;
    });
    ...
    rawData.forEach(r => {
      ...
      if (r.fecha >= priorStart && r.fecha <= priorEnd) {
        ...
        if (key === 'totalExams') priorCount += r.examsCount;
        if (key === 'urgentCount' && r.procedencia_de_derivaci_n === 'Urgencia') priorCount++;
        if (key === 'closedCount' && r.procedencia_de_derivaci_n === 'Atención Cerrada') priorCount++;
      }
    });
    ...
    filteredData.forEach(r => {
      if (r.establecimiento_de_origen && r.fecha !== 'Sin Fecha' && r.examsCount > 0) {
        ...
        r.examsList.forEach(exam => {
          ...
          let cat = 'Otros';
          if (p.includes('cráneo') || p.includes('craneo') || p.includes('encefál') || p.includes('encefal') || p.includes('hipotálamo') || p.includes('fosa') || p.includes('temporal') || p.includes('oído') || p.includes('órbita') || p.includes('cuello')) cat = 'Cerebral y Cuello';
          else if (p.includes('abdomen') || p.includes('pelvis') || p.includes('tórax') || p.includes('torax') || p.includes('pielografía') || p.includes('urografía')) cat = 'Abdomen/Tórax';
          else if (p.includes('angio')) cat = 'Angiotomografías';
          else if (p.includes('columna') || p.includes('cervical') || p.includes('dorsal') || p.includes('lumbar') || p.includes('musculoesquelética')) cat = 'Osteoarticular';
  ...
  const examCategories = useMemo(() => {
    const counts = {
      brain_neck: { label: 'Cerebral, Facial y Cuello', count: 0, list: {} },
      body_chest: { label: 'Abdomen, Pelvis y Tórax', count: 0, list: {} },
      angio: { label: 'Angiotomografías (Angio-TAC)', count: 0, list: {} },
      spine_musculoskeletal: { label: 'Osteoarticular y Columna', count: 0, list: {} }
    };
    ...
    filteredData.forEach(r => {
      r.examsList.forEach(exam => {
        ...
        if (p.includes('angio')) {
          cat = 'angio';
        } else if (p.includes('cráneo') || p.includes('craneo') || p.includes('encefál') || p.includes('encefal') || p.includes('hipotálamo') || p.includes('hipofisis') || p.includes('fosa posterior') || p.includes('temporal') || p.includes('oído') || p.includes('oido') || p.includes('órbita') || p.includes('orbita') || p.includes('maxilofacial') || p.includes('cuello') || p.includes('partes blandas')) {
          cat = 'brain_neck';
        } else if (p.includes('abdomen') || p.includes('pelvis') || p.includes('tórax') || p.includes('torax') || p.includes('pielografía') || p.includes('pielografia') || p.includes('urografía') || p.includes('urografia') || p.includes('colonoscopía') || p.includes('enterografía') || p.includes('enterografia')) {
          cat = 'body_chest';
        } else if (p.includes('columna') || p.includes('cervical') || p.includes('dorsal') || p.includes('lumbar') || p.includes('musculoesquelética') || p.includes('musculoesqueltica') || p.includes('articulación') || p.includes('rodilla') || p.includes('pie') || p.includes('tobillo') || p.includes('hombro') || p.includes('codo') || p.includes('mano')) {
          cat = 'spine_musculoskeletal';
        }
  ...
  const remData = useMemo(() => {
    ...
    const isOperativo = (r) => {
      ...
      return modal.includes('convenio') || cc.includes('631') || modal.includes('operativo') || cc.includes('operativo');
    };
  ...
  return (
    <div className="radiology-portal">
      {/* Header Panel - Neon Purple style matching light theme layout */}
      <header className="portal-header neon-purple">
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          <button className="circle-back-btn purple-back" onClick={onBack} title="Volver al portal estratégico">
            <ArrowLeft size={22} />
          </button>
          <div>
            <div className="header-badges">
              <span className="live-status purple-live"><span className="pulse-dot purple-dot"></span> REDCap Integrado</span>
              <span className="api-badge purple-api">API Araucanía Sur</span>
              <span className="update-badge purple-update">📅 Carga Semanal: {lastUpdated}</span>
            </div>
            <h1 className="portal-title text-glow-purple">Producción de TAC</h1>
            <p className="portal-subtitle">Monitoreo de Tomografía Axial Computarizada y derivaciones • Villarrica</p>
          </div>
        </div>
      </header>

      {/* Tabs Menu - Purple Variant */}
      <div className="portal-tabs purple-tabs">
        ...
        >
          ...
          <div className="filter-item" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            <div>
              ...
              <input 
                ...
                style={{ width: '100%', padding: '10px', borderRadius: '10px', border: '1px solid rgba(168, 85, 247, 0.2)', background: 'rgba(255,255,255,0.02)', color: '#0f172a', fontSize: '0.8rem', outline: 'none' }}
              />
            </div>
            <div>
              <label>HASTA</label>
          ...
          <div className="filter-item">
            ...
            <div style={{ position: 'relative' }}>
              <input 
                type="text" 
                placeholder="Ej: Cráneo, Abdomen, Angio..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                style={{ width: '100%', padding: '12px 12px 12px 34px', borderRadius: '12px', border: '1.5px solid rgba(168, 85, 247, 0.15)', background: 'white', fontWeight: '600', color: '#1e293b', outline: 'none' }}
              />
              <Search size={16} style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
            </div>
          </div>

          <div className="sidebar-stats purple-stats">
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ fontWeight: 600, fontSize: '0.8rem', color: 'rgba(0,0,0,0.4)' }}>Registros Filtrados</span>
              <span style={{ fontWeight: 800, color: '#a855f7' }}>{filteredData.length.toLocaleString()}</span>
            </div>
            <div className="sidebar-progress-bg" style={{ background: 'rgba(168, 85, 247, 0.1)' }}>
              <div className="sidebar-progress-fill" style={{ width: `${rawData.length > 0 ? (filteredData.length / rawData.length) * 100 : 0}%`, background: 'linear-gradient(90deg, #a855f7, #c084fc)' }}></div>
            </div>
        ...
        <main className="portal-content-pane">
          {loading ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '400px', gap: '15px' }}>
              ...
              <p style={{ color: 'rgba(0,0,0,0.5)', fontWeight: 700 }}>Procesando base de datos de tomografías (25,619 registros)...</p>
            </div>
          ) : error ? (
            ...
            <>
              ...
              <AnimatePresence mode="wait">
                ...
                {activeTab === 'summary' && (
                  ...
                  <div className="metrics-summary-bar">
                    {(() => {
                      const yoyTotal = getYoYStats('total');
                      const yoyExams = getYoYStats('totalExams');
                      const yoyUrgent = getYoYStats('urgentCount');
                      const yoyClosed = getYoYStats('closedCount');

                      return (
                        <>
                          <div className="metric-box glass-card border-purple">
                            <span className="m-label">TOTAL PROCEDIMIENTOS</span>
                            <h3 className="m-value" style={{ color: '#8b5cf6' }}>{stats.total.toLocaleString()}</h3>
                            <p className="m-desc">Atenciones/visitas agendadas</p>
                            <div className={`yoy-indicator ${yoyTotal.trend}`} style={{ fontSize: '0.74rem', fontWeight: '800', marginTop: '8px', display: 'flex', alignItems: 'center', gap: '4px', color: yoyTotal.trend === 'positive' ? '#2e7d32' : (yoyTotal.trend === 'negative' ? '#c62828' : '#64748b') }}>
                              {yoyTotal.text}
                            </div>
                          </div>
                          ...
                          <div className="metric-box glass-card border-violet">
                            ...
                            <h3 className="m-value" style={{ color: '#a855f7' }}>{stats.totalExams.toLocaleString()}</h3>
                            <p className="m-desc">Exámenes en las 4 columnas</p>
                            <div className={`yoy-indicator ${yoyExams.trend}`} style={{ fontSize: '0.74rem', fontWeight: '800', marginTop: '8px', display: 'flex', alignItems: 'center', gap: '4px', color: yoyExams.trend === 'positive' ? '#2e7d32' : (yoyExams.trend === 'negative' ? '#c62828' : '#64748b') }}>
                              {yoyExams.text}
                            </div>
                          </div>
                          ...
                          <div className="metric-box glass-card border-pink">
                            ...
                            <h3 className="m-value" style={{ color: '#ec4899' }}>{stats.urgentCount.toLocaleString()}</h3>
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '8px', marginTop: '8px' }}>
                              <span style={{ fontSize: '0.68rem', fontWeight: 800, padding: '2px 8px', borderRadius: '10px', background: 'rgba(236, 72, 153, 0.1)', color: '#ec4899' }}>
                                {stats.total > 0 ? ((stats.urgentCount / stats.total) * 100).toFixed(1) : 0}% de carga
                              </span>
                              <div className={`yoy-indicator ${yoyUrgent.trend}`} style={{ fontSize: '0.74rem', fontWeight: '800', display: 'flex', alignItems: 'center', gap: '2px', color: yoyUrgent.trend === 'positive' ? '#2e7d32' : (yoyUrgent.trend === 'negative' ? '#c62828' : '#64748b') }}>
                                {yoyUrgent.text}
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="metric-box glass-card border-emerald">
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '8px', marginTop: '8px' }}>
                              <span style={{ fontSize: '0.68rem', fontWeight: 800, padding: '2px 8px', borderRadius: '10px', background: 'rgba(16, 185, 129, 0.1)', color: '#10b981' }}>
                                {stats.total > 0 ? ((stats.closedCount / stats.total) * 100).toFixed(1) : 0}% de carga
                              </span>
                              <div className={`yoy-indicator ${yoyClosed.trend}`} style={{ fontSize: '0.74rem', fontWeight: '800', display: 'flex', alignItems: 'center', gap: '2px', color: yoyClosed.trend === 'positive' ? '#2e7d32' : (yoyClosed.trend === 'negative' ? '#c62828' : '#64748b') }}>
                                {yoyClosed.text}
                              </div>
                            </div>
                          </div>
                  ...
                  <div className="glass-card chart-container border-glow-purple">
                    <div className="chart-header">
                      <div>
                        <h2 className="c-title text-glow-purple">Volumen Mensual de Escáner (TAC)</h2>
                        <p className="c-subtitle">Distribución secuencial de producción y tasa de inasistencia (NSP %)</p>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                        <div className="legend-item"><span className="legend-color" style={{ background: 'linear-gradient(180deg, #8b5cf6, #c084fc)' }}></span> Producción Realizada</div>
                          ...
                          <svg viewBox="0 0 1000 220" width="100%" height="100%" preserveAspectRatio="none" style={{ overflow: 'visible' }}>
                            ...
                            {(() => {
                              ...
                              return (
                                <React.Fragment>
                                  {/* Area Fill */}
                                  {areaPath && <path d={areaPath} fill="url(#tacAreaGrad)" opacity="0.15" />}
                                  
                                  {/* Production Line */}
                                  {pathData && <path d={pathData} fill="none" stroke="#8b5cf6" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" style={{ filter: 'drop-shadow(0 4px 8px rgba(139,92,246,0.3))' }} />}
                                  ...
                                  <defs>
                                    <linearGradient id="tacAreaGrad" x1="0" y1="0" x2="0" y2="1">
                                  ...
                                  {points.map((p, i) => (
                                    <g key={i} className="chart-interactive-group" style={{ cursor: 'pointer' }}>
                                      <circle cx={p.x} cy={p.y} r="5" fill="#8b5cf6" stroke="#fff" strokeWidth="2" />
                                      <text x={p.x} y={p.y - 12} fill="#4c1d95" fontSize="9" fontWeight="800" textAnchor="middle">{p.total}</text>
                  ...
                  <div className="summary-bottom-row">
                    {/* NSP & lost capacity */}
                    <div className="glass-card bottom-card border-glow-purple">
                      <h3 className="bc-title" style={{ color: '#0f172a', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <Clock size={18} style={{ color: '#8b5cf6' }} /> Impacto del Ausentismo (NSP)
                      </h3>
                      <div className="attendance-gauge" style={{ borderBottomColor: 'rgba(168, 85, 247, 0.1)' }}>
                        <div className="gauge-val" style={{ color: '#8b5cf6' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <span style={{ fontSize: '0.8rem', fontWeight: 600, color: '#64748b' }}>Horas de Tomógrafo Perdidas</span>
                    ...
                    <div className="glass-card bottom-card border-glow-pink" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                      ...
                      <p style={{ fontSize: '0.82rem', color: '#64748b', lineHeight: '1.6', marginBottom: '16px' }}>
                        El establecimiento derivador con mayor número absoluto de inasistencias en el tomógrafo (TAC):
                      </p>
              ...
              {activeTab === 'pivot' && (
                <motion.div key="pivot" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="glass-card pivot-card border-glow-purple">
                  <div className="pivot-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
                    <div>
                      <h3 className="pivot-title" style={{ color: '#0f172a', margin: 0 }}>Matriz de Producción de TAC (Establecimiento vs Mes)</h3>
                      <p className="pivot-subtitle" style={{ color: '#64748b', marginTop: '4px' }}>
                        Muestra la sumatoria total de **exámenes realizados** (recorriendo las 4 columnas) por establecimiento derivador.
                      </p>
                    </div>
                    <span className="live-status" style={{ background: 'rgba(168, 85, 247, 0.08)', color: '#a855f7', fontSize: '0.78rem', fontWeight: 800, padding: '6px 14px', borderRadius: '12px' }}>
                    ...
                    <table className="pivot-table">
                      <thead>
                        <tr>
                          ...
                          <th style={{ color: '#a855f7' }}>Total</th>
                        </tr>
                      </thead>
                      <tbody>
                          ...
                          <React.Fragment key={row.name}>
                            <tr className="main-row" style={{ cursor: 'pointer' }} onClick={() => toggleEstablishment(row.name)}>
                              <td className="sticky-cell est-name-cell" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                <ChevronRight size={14} style={{ transition: 'transform 0.2s', transform: expandedEstablishments[row.name] ? 'rotate(90deg)' : 'none', color: '#a855f7' }} />
                                {row.name}
                              </td>
                              {pivotMatrix.columns.map(col => (
                                <td key={col} className="data-cell">{(row.months[col] || 0).toLocaleString()}</td>
                              ))}
                              <td className="row-total-cell" style={{ fontWeight: 800, color: '#a855f7' }}>{row.totalExams.toLocaleString()}</td>
                            </tr>
                      ...
                      {pivotMatrix.rows.length > 0 && (
                        <tfoot>
                          <tr className="total-row" style={{ borderTop: '2px solid #a855f7' }}>
                            ...
                            <td className="grand-total-cell" style={{ color: '#a855f7', fontWeight: 900 }}>
                ...
                <motion.div key="classification" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="tab-pane-layout">
                  <div className="details-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
                    {Object.entries(examCategories).map(([key, cat]) => {
                      const icons = {
                        brain_neck: <Layers size={24} style={{ color: '#8b5cf6' }} />,
                        body_chest: <Activity size={24} style={{ color: '#ec4899' }} />,
                        angio: <Heart size={24} style={{ color: '#ef4444' }} />,
                        spine_musculoskeletal: <Users size={24} style={{ color: '#10b981' }} />
                      };

                      const borderColors = {
                        brain_neck: 'border-glow-purple',
                        body_chest: 'border-glow-pink',
                        angio: 'border-glow-pink',
                        spine_musculoskeletal: 'border-glow-emerald'
                      };
                      ...
                      return (
                        <div key={key} className={`glass-card detail-sub-card ${borderColors[key]}`} style={{ padding: '24px' }}>
                          ...
                          <p style={{ color: '#64748b', fontSize: '0.8rem', margin: '0 0 16px 0', lineHeight: '1.4' }}>
                            Distribución de tomografías en la red del hospital Villarrica.
                          </p>
              ...
              {activeTab === 'rem' && (
                <motion.div key="rem" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} className="glass-card border-glow-purple" style={{ padding: '30px' }}>
                  <div style={{ marginBottom: '24px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '15px' }}>
                      <div>
                        <h3 className="pivot-title" style={{ color: '#0f172a', margin: 0 }}>Resumen Estadístico Mensual (REM) - Tomografía</h3>
                        <p className="pivot-subtitle" style={{ color: '#64748b', marginTop: '4px' }}>
                          Consolidado de prestaciones realizadas según previsión, procedencia clínica y convenios operativos.
                        </p>
                      </div>
                      <span className="live-status" style={{ background: 'rgba(168, 85, 247, 0.08)', color: '#8b5cf6', fontSize: '0.74rem', fontWeight: 800, padding: '6px 14px', borderRadius: '12px' }}>
                  ...
                  <div className="pivot-table-wrapper" style={{ boxShadow: '0 4px 20px rgba(0,0,0,0.02)' }}>
                    <table className="pivot-table">
                      <thead>
                        <tr>
                          <th className="sticky-cell" style={{ textAlign: 'left', minWidth: '280px' }}>Tipo de Tomografía Realizada</th>
                          <th>Beneficiarios (FONASA)</th>
                          <th>No Beneficiarios</th>
                          <th>Atención Cerrada</th>
                          <th>Atención Abierta</th>
                          <th>Urgencia</th>
                          <th style={{ background: 'rgba(168, 85, 247, 0.04)', color: '#a855f7', fontWeight: 800 }}>Convenio/Operativos</th>
                          <th style={{ background: 'rgba(139, 92, 246, 0.04)', color: '#8b5cf6', fontWeight: 800 }}>Total Realizado</th>
                        </tr>
                      </thead>
                      <tbody>
                        {paginatedRemData.length === 0 ? (
                          <tr>
                            <td colSpan={8} style={{ textAlign: 'center', padding: '50px', color: '#64748b', fontSize: '0.9rem', fontWeight: 600 }}>
                              No hay tomografías realizadas registradas para los filtros seleccionados.
                            </td>
                          </tr>
                        ) : (
                          paginatedRemData.map(row => (
                            <tr key={row.name} className="main-row">
                              ...
                              <td className="row-total-cell" style={{ fontWeight: 800, background: 'rgba(139, 92, 246, 0.02)' }}>
                      ...
                      {remData.length > 0 && (
                        <tfoot>
                          <tr className="total-row">
                            ...
                            <td className="grand-total-cell" style={{ fontWeight: 900, color: '#8b5cf6', background: 'rgba(139, 92, 246, 0.05)' }}>
                    ...
                    <div className="glass-card border-glow-purple" style={{ padding: '20px', flex: 1, minWidth: '280px', display: 'flex', alignItems: 'center', gap: '18px', background: 'rgba(168, 85, 247, 0.02)' }}>
                      ...
                      <div>
                        <span style={{ fontSize: '0.74rem', fontWeight: 800, color: '#a855f7', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                          Tomografías por Convenio
                        </span>
                        <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#475569', lineHeight: '1.4' }}>
                          Se identificaron <strong style={{ color: '#a855f7', fontSize: '0.95rem', fontWeight: 900 }}>{remData.reduce((sum, r) => sum + r.operativos, 0).toLocaleString()} exámenes</strong> en convenios de integración de la red y operativos especiales de reducción de listas.
                        </p>
                      </div>
                    </div>
                  </div>
      ...
      {/* Styled JSX block - Neon Purple Light Theme Edition */}
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .portal-tab.active {
          color: #8b5cf6;
          border-bottom: 3.5px solid #8b5cf6;
          background: rgba(168, 85, 247, 0.05);
        ...
        .metric-box.border-purple {
          border: 1px solid rgba(139, 92, 246, 0.2) !important;
          border-left: 6px solid #8b5cf6 !important;
          box-shadow: 0 8px 32px 0 rgba(139, 92, 246, 0.03) !important;
        }

        .metric-box.border-violet {
          border: 1px solid rgba(168, 85, 247, 0.2) !important;
          border-left: 6px solid #a855f7 !important;
          box-shadow: 0 8px 32px 0 rgba(168, 85, 247, 0.03) !important;
        }

        .metric-box.border-pink {
          border: 1px solid rgba(236, 72, 153, 0.2) !important;
          border-left: 6px solid #ec4899 !important;
          box-shadow: 0 8px 32px 0 rgba(236, 72, 153, 0.03) !important;
        }

        .metric-box.border-emerald {
          border: 1px solid rgba(16, 185, 129, 0.2) !important;
