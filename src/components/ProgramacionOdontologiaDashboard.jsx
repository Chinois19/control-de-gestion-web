...
import { ChevronDown, ChevronRight, ArrowLeft, Filter, AlertTriangle, Clock, Activity, Users } from 'lucide-react';
import { programacionOdontologia } from '../data/programacionVillarrica2026';
...
const ESP_COLORS = [
  '#6366f1','#0ea5e9','#10b981','#f59e0b','#ef4444','#8b5cf6','#ec4899','#14b8a6','#f97316'
];
    ...
    <span style={{ position: 'relative', display: 'inline-block' }}
      ...
      {show && (
        <div style={{
          ...
          boxShadow: '0 8px 32px rgba(0,0,0,0.4)', fontSize: '0.75rem', lineHeight: '1.6', marginBottom: '8px',
        }}>
          <div style={{ fontWeight: 800, color: '#818cf8', fontSize: '0.7rem', textTransform: 'uppercase', marginBottom: '8px' }}>📋 Cómo registrar en el aplicativo</div>
          {glosa.actPlanilla && <div style={{ marginBottom: '4px' }}><span style={{ color: '#94a3b8' }}>Planilla: </span><span style={{ color: '#e2e8f0', fontWeight: 600 }}>{glosa.actPlanilla}</span></div>}
          {glosa.grupoRCE && glosa.grupoRCE !== 'Sin RCE' && <div style={{ marginBottom: '4px' }}><span style={{ color: '#94a3b8' }}>Grupo RCE: </span><span style={{ color: '#e2e8f0' }}>{glosa.grupoRCE}</span></div>}
          {glosa.actRCE && glosa.actRCE !== 'Sin RCE' && <div style={{ marginBottom: '4px' }}><span style={{ color: '#94a3b8' }}>Actividad RCE: </span><span style={{ color: '#a5b4fc', fontWeight: 600 }}>{glosa.actRCE}</span></div>}
          {glosa.rem && glosa.rem !== 'N/A' && <div><span style={{ color: '#94a3b8' }}>Serie REM: </span><span style={{ color: '#6ee7b7' }}>{glosa.rem}</span></div>}
          {glosa.descripcion && <div style={{ marginTop: '8px', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '8px', color: '#94a3b8', fontSize: '0.7rem' }}>{glosa.descripcion.substring(0, 200)}{glosa.descripcion.length > 200 ? '...' : ''}</div>}
        </div>
      )}
    </span>
  );
...
function ActividadesTable({ actividades, filterRR }) {
  ...
  if (!filtered.length) return <div style={{ padding: '16px', color: '#94a3b8', fontSize: '0.82rem', textAlign: 'center' }}>No hay actividades para el filtro</div>;
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
        <thead>
          <tr style={{ background: 'rgba(99,102,241,0.06)' }}>
            {['Actividad', 'Proceso', 'Horas', 'Rendim.', 'Prod. 1°Sem', 'Prod. 2°Sem', 'Total', 'R.R.'].map(h => (
          ...
          {filtered.map((act, i) => {
            ...
            return (
              <tr key={i} style={{ borderBottom: '1px solid #f1f5f9', background: i % 2 === 0 ? 'transparent' : 'rgba(248,250,252,0.5)' }}>
                <td style={{ padding: '8px 12px', maxWidth: '260px' }}><GlosaTooltip glosa={act.glosa} actividad={act.actividad} /></td>
                <td style={{ padding: '8px 12px' }}>{act.proceso ? <span style={{ background: procCfg.bg, color: procCfg.color, padding: '2px 8px', borderRadius: '20px', fontSize: '0.68rem', fontWeight: 600 }}>{act.proceso}</span> : '—'}</td>
  ...
  return (
    <div style={{ borderBottom: '1px solid #f1f5f9' }}>
      <div onClick={() => setOpen(!open)} style={{
        ...
        gap: '12px', background: open ? 'rgba(99,102,241,0.04)' : 'transparent', transition: 'background 0.2s',
      }}>
...
function EspecialidadRow({ esp, filterRR, color }) {
  const [open, setOpen] = useState(false);
  ...
  return (
    <div style={{ border: `1px solid ${color}22`, borderLeft: `4px solid ${color}`, borderRadius: '12px', overflow: 'hidden', marginBottom: '10px', background: 'white' }}>
      <div onClick={() => setOpen(!open)} style={{
        ...
        background: open ? `${color}08` : 'white', gap: '12px', transition: 'background 0.2s',
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <ChevronDown size={16} style={{ transform: open ? 'rotate(180deg)' : 'rotate(-90deg)', transition: '0.2s', color, flexShrink: 0 }} />
          <div>
            <div style={{ fontSize: '0.95rem', fontWeight: 800, color: '#1e293b' }}>{esp.nombre}</div>
            <div style={{ fontSize: '0.72rem', color: '#64748b', marginTop: '2px' }}>
              {esp.numProfesionales} odontólogo{esp.numProfesionales !== 1 ? 's' : ''}
              {pctR > 0 && <span style={{ color: '#10b981', marginLeft: '8px' }}>• {pctR}% evaluable</span>}
            </div>
          </div>
        </div>
        <span style={{ fontSize: '0.88rem', fontWeight: 700, color, textAlign: 'right' }}>{fmtH(esp.totalHorasAsignadas)}</span>
        ...
        <span style={{ fontSize: '0.92rem', fontWeight: 800, color: '#1e293b', textAlign: 'right' }}>{fmt(esp.totalProd)}</span>
      ...
      {open && (
        <div style={{ background: '#fafbfc', borderTop: `1px solid ${color}22` }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 90px 90px 90px 90px 100px', gap: '12px', padding: '6px 16px', background: `${color}06`, borderBottom: '1px solid #e2e8f0' }}>
            {['Odontólogo/a', 'Horas Asig.', '1° Sem', '2° Sem', 'Total', 'Total R', 'R.R.'].map(h => (
              <span key={h} style={{ fontSize: '0.68rem', fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', textAlign: h !== 'Odontólogo/a' ? 'right' : 'left' }}>{h}</span>
            ))}
          </div>
...
export default function ProgramacionOdontologiaDashboard({ onBack }) {
  ...
  const totalProfs = programacionOdontologia.reduce((s, e) => s + e.numProfesionales, 0);
  const totalHoras = programacionOdontologia.reduce((s, e) => s + e.totalHorasAsignadas, 0);
  const totalProd = programacionOdontologia.reduce((s, e) => s + e.totalProd, 0);
  const totalProd1 = programacionOdontologia.reduce((s, e) => s + e.prod1Sem, 0);
  const totalProd2 = programacionOdontologia.reduce((s, e) => s + e.prod2Sem, 0);
  const totalR = programacionOdontologia.flatMap(e => e.profesionales).flatMap(p => p.actividades)
    .filter(a => a.rr === 'R').reduce((s, a) => s + a.prod1Sem + a.prod2Sem, 0);

  const filtered = useMemo(() => {
    if (!searchEsp.trim()) return programacionOdontologia;
    const q = searchEsp.toLowerCase();
    return programacionOdontologia.filter(e => e.nombre.toLowerCase().includes(q));
  }, [searchEsp]);

  const kpis = [
    { label: 'Especialidades', value: totalEsp },
    { label: 'Odontólogos', value: fmt(totalProfs) },
    { label: 'Horas Asignadas', value: fmtH(totalHoras) },
    { label: 'Total Comprometido', value: fmt(totalProd) },
    { label: '1° Semestre', value: fmt(totalProd1) },
    { label: '2° Semestre', value: fmt(totalProd2) },
    { label: 'Total Evaluable (R)', value: fmt(totalR) },
  ];
  ...
  return (
    <div style={{ fontFamily: "'Inter', 'Segoe UI', sans-serif", minHeight: '100vh', background: '#f8fafc' }}>
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', top: 0, right: 0, width: '300px', height: '100%', background: 'radial-gradient(ellipse at right, rgba(6,182,212,0.3) 0%, transparent 70%)' }} />
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '20px' }}>
          <div style={{ background: 'rgba(255,255,255,0.15)', borderRadius: '16px', padding: '16px', backdropFilter: 'blur(10px)', fontSize: '32px' }}>
            🦷
          </div>
          <div>
            ...
            <h1 style={{ fontSize: '2rem', fontWeight: 900, margin: 0, letterSpacing: '-0.5px' }}>
              Programación Odontología
            </h1>
            <p style={{ color: 'rgba(255,255,255,0.7)', marginTop: '8px', fontSize: '0.9rem' }}>
              Programación odontólogos especialistas 2026 · Archivo: P 2026 Odontología.xlsx
            </p>
          </div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '16px', marginTop: '28px' }}>
          {kpis.map((k, i) => (
            <div key={i} style={{ background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(10px)', borderRadius: '14px', padding: '16px', border: '1px solid rgba(255,255,255,0.15)' }}>
      ...
      <div style={{ margin: '24px 40px 0', background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.25)', borderRadius: '12px', padding: '14px 18px', display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
        ...
        <div style={{ fontSize: '0.82rem', color: '#92400e', lineHeight: '1.6' }}>
          ...
          Las actividades <RRBadge rr="NR" size="sm" /> <strong>no serán contempladas en la evaluación final del profesional</strong> según orientaciones técnicas MINSAL.
        </div>
      </div>
      ...
      <div style={{ margin: '20px 40px', display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '6px', background: 'white', padding: '4px', borderRadius: '10px', border: '1px solid #e2e8f0' }}>
          {['todos', 'R', 'NR'].map(v => (
            <button key={v} onClick={() => setFilterRR(v)} style={{
              ...
              background: filterRR === v ? '#0e7490' : 'transparent',
        ...
        <div style={{ marginLeft: 'auto', fontSize: '0.82rem', color: '#94a3b8' }}>
          {filtered.length} especialidades · {filtered.reduce((s, e) => s + e.numProfesionales, 0)} odontólogos
        </div>
      </div>

      {/* Header tabla */}
      <div style={{ margin: '0 40px 4px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 90px 90px 90px 90px 100px', gap: '12px', padding: '8px 16px', background: 'rgba(14,116,144,0.06)', borderRadius: '8px 8px 0 0', border: '1px solid #e2e8f0', borderBottom: 'none' }}>
          {['Especialidad / Odontólogo', 'Horas Asig.', '1° Sem', '2° Sem', 'Total', 'Total R', 'R.R.'].map(h => (
      ...
      <div style={{ margin: '0 40px', paddingBottom: '60px' }}>
        {filtered.map((esp, idx) => (
          <EspecialidadRow key={esp.nombre} esp={esp} filterRR={filterRR} color={ESP_COLORS[idx % ESP_COLORS.length]} />
        ))}
      </div>
    </div>
  );
